...
describe("Renditions KPIs", () => {
  ...
  test("every TypeScript event name is in the database allowlist", () => {
    // The latest migration that re-emits the event_name CHECK constraint.
    const migration = readFileSync(new URL("../../supabase/migrations/20260918110000_engagement_signal_analytics.sql", import.meta.url), "utf8");
    assert.ok(PRODUCT_EVENT_NAMES.includes("signup_from_invite") && PRODUCT_EVENT_NAMES.length >= 30, "event list did not parse");
