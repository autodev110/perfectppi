...
-- ============================================================================
-- Wire the report_auto_hide flag (plan 3.4 / 30.2 / 39). Flags are resolved
-- per environment in the application, so the report transaction takes the
-- decision as a parameter. The seven-argument form stays as a wrapper that
-- keeps today's behavior (auto-hide on) for any caller that has not been
-- updated.
-- ============================================================================
...
BEGIN
  ...
  IF v_case.id IS NULL THEN
    ...
    -- report_auto_hide (plan 3.4 / 30.2): the server decides whether the
    -- first valid report hides content globally. With the flag off, an
    -- ordinary report opens a monitoring case and the content stays up;
    -- urgent categories still hide regardless.
    v_should_hide := (NOT v_previous_restored AND COALESCE(p_auto_hide, true)) OR v_priority = 'urgent';
...
AS $$
  SELECT public.submit_moderation_report(
    p_reporter_id, p_entity_type, p_entity_id, p_revision_id, p_reason_code, p_details, p_idempotency_key, true
  );
$$;

REVOKE ALL ON FUNCTION public.submit_moderation_report(uuid, text, uuid, uuid, text, text, text, boolean)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.submit_moderation_report(uuid, text, uuid, uuid, text, text, text, boolean)
  TO service_role;
