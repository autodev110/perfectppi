-- ============================================================================
-- Migration: allow organization managers to hold a DealerSpace user link.
--
-- The exchange previously required role = 'technician'. At a small dealership
-- the person who connects DealerSpace is often the same person who inspects the
-- car, and refusing them a link left no way to send a vehicle at all.
--
-- This widens *who may be assigned an inspection*, not what a linked account can
-- reach. Managers already hold a technician_profiles row carrying the
-- organization membership this function checks, and every RLS policy on the
-- performer path keys on profile id rather than role — so the same-organization
-- requirement below is still the whole of the authorization.
--
-- Consumers and admins remain ineligible.
--
-- Redefines public.partner_exchange_user_link from
-- 20260816233245_partner_user_link_exchange_atomic.sql; the body is otherwise
-- unchanged.
-- ============================================================================
...
BEGIN
  ...
  IF NOT FOUND
     OR v_profile_role NOT IN ('technician', 'org_manager')
