...
| First-party product analytics | Successful server-confirmed product actions; understand activation and useful vehicle activity | Service-only `product_analytics_events`; aggregate-only admin dashboard | Supabase; authorized PerfectPPI administrators see aggregates | Account cascade; immediate deletion on opt-out; automatic expiry after 90 days; no content, VIN, location, or arbitrary property payload |
