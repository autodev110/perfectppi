...
**Status:** Repository implementation and release validation register; production sign-off pending
...
**Implementation status (September 18, 2026):** authors edit their own live post text on web and iOS (`edit_community_post`); the existing revision trigger records the previous wording as an immutable revision, `edited_at` drives the "Edited" label on cards, detail, and My Posts, and the edit is refused while any non-closed moderation case is bound to the post — a restored post (closed case) becomes editable again and the next report binds to the new revision. Edited text passes the same launch policy as a new post. Archive, restore, and the 30-day purge were already in place.
...
**Implementation status (September 18, 2026):** comments support one level of replies (`parent_comment_id`, parent bound to the same post and never itself a reply; a reply to a removed or hidden comment is refused), with the parent's author notified through an aggregated `comment_reply` notice that deep-links to the reply. Authors edit an active, unreported comment (`edit_community_comment`, immutable revision, "Edited" label) and remove their own comment softly (`remove_own_community_comment` → archived; the row, revisions, and any case stay). A removed or hidden top-level comment with live replies renders as a neutral "Comment removed" placeholder on web and iOS; without replies it is omitted. Accepted Answer and Helpful apply to top-level responses only. Blocked members cannot reply to each other.
...
**Implementation status (September 18, 2026):** the web and iOS composers show the anti-scam caution as soon as a draft contains an email or a phone-shaped number (`src/lib/messages/contact-warning.ts`, `ContactDetailsDetector`); it is a caution, never a block, and prices or mileage do not trigger it. Delete-for-self semantics are explicitly defined for this release: no per-message or per-thread deletion control is offered. Messages remain available to authorized participants subject to account-deletion, retention, and evidence rules; blocking hides the thread from the blocker's ordinary inbox without recalling the recipient's copy. Account deletion follows the participant-aware privacy worker and is not a message-unsend feature. Adding an independent delete-for-self feature requires a later product decision.
...
**Implementation status (September 18, 2026):** the remaining web page/control headings, labels, placeholders, accessibility labels, empty states, policy copy, and display errors are extracted to `src/lib/i18n/messages/en-ui.ts`, merged with the existing named resources in `messages/en.ts`. Navigation and shared role, group, post-type, event, and credential labels are resource-backed too. Client components use `useTranslator()` and asynchronous server pages use `getRequestTranslator()`; request-free helpers and module-level option data retain the default-language shorthand. Root locale negotiation is wired, with a single-catalog fast path that preserves static public rendering. Shared date, number, currency, and relative-time formatters accept a locale. Backend codes, form field names, inspection-template join keys, routes, and protocol values remain stable; stored user content is never translated by this extraction. `npm run i18n:check` detects new extractable screen literals; unit tests check keys, interpolation parameters, protocol-input exclusions, and disclosure copy. The versioned accepted Terms snapshot retains its original assent hash and a separate test checks unchanged English wording. All 225 refactored screen/shared-copy modules preserve their pre-refactor English/protocol literals. iOS remains compiler-extracted and `scripts/sync-string-catalog.py --check` covers all 1,319 strings. Only English ships: a new language still needs translated catalogs, locale binding for default-language option/helper output, legal translation approval, and real text-expansion/accessibility acceptance before it is advertised.

## 33. Reliability and performance

**Implementation status (September 14, 2026):** the main Community feed, every unified-search result type, the dedicated People and Marketplace directories, saved Community/Marketplace content, and group posts/search/members/FAQ now use bounded, stable cursor pagination on web and iOS while retaining legacy offset responses for installed app compatibility. Feed pages preserve visibility, mutes, filters, and repost collapsing; search cursors preserve relevance ordering and are bound to the normalized query and result type. Saved-content keysets preserve private save order, group cursors are bound to the group and normalized query, and Marketplace cursors preserve every browse filter and all six sort modes. iOS supports incremental loading/retry and both clients show immediate loading states. The admin dashboard exposes allowlisted aggregate query timings from normalized PostgreSQL statistics, and R2 operations emit identifier-free slow/failure events. Community images use metadata-free bounded server derivatives; web defers off-screen image decode, and iOS uses cancellation-aware downsampling with a cost-limited shared cache. Production-shaped load testing and budgets, deployment alert routing, and measured server-variant/network-byte tuning remain open.
...
**Implementation status (September 14, 2026):** the first-party analytics foundation is implemented with server-confirmed meaningful-action events, account-level opt-out and immediate deletion, a 90-day retention worker, and aggregate-only admin dashboards. Product reporting now covers friend-request acceptance, question response/accepted-answer rates, technical saves, retained group participation, D7/D30 meaningful return, and independent Marketplace funnel stages. Safety reporting now covers report volume per thousand published items, privacy-thresholded reason/surface cohorts, median/P95 review time, moderation and appeal outcomes, repeat patterns, block/mute adoption, notification opt-out, queue/SLA health, hidden public media references, and visibility-integrity violations. The function returns no account, content, report, case, or media identifiers and does not read report text into its output. Production beta baselines and release targets remain operational work.

**Implementation status (September 18, 2026, audited):** engagement signals use the closed event allowlist, account opt-out, 90-day expiry, and aggregate-only output (`get_product_engagement_signals`, on the admin analytics page). Group discovery-to-join matches a view and a later join of the same group using an opaque hash. Upload completion matches each reservation to a successfully saved attachment for that same upload; validation or row-save failures do not count as completions. Unwanted-contact attribution counts profile/message reports and block-refused contact attempts; the message-volume denominator excludes opted-out senders. iOS submits bounded foreground-session observations and received MetricKit crash diagnostics without stacks or device details. Delayed diagnostics are not session-attributed: the product owner explicitly chose to keep MetricKit and defer exact crash-free session rates (September 18, 2026). No exact rate is displayed, no new crash provider is added, and diagnostics divided by foreground observations must not be presented as that rate. Intent segmentation uses account role and first-week actions, with D7/D30 rates withheld for cohorts under five accounts. Web sessions/crashes are not measured.
...
**Approved measurement deferral (September 18, 2026):** keep the existing privacy-minimized MetricKit integration; defer exact session-attributed crash-free rates. This is a deliberate scope decision, not an unfinished provider integration.
...
**Repository status (September 18, 2026):** author editing, one-level replies, soft comment removal, the contact-sharing caution, engagement observations, and the audit protections above are implemented. The remaining screen-level resource extraction is now implemented (32.2); message retention/no-delete-for-self semantics are defined (22.3); exact crash-free rates are explicitly deferred by the product owner (34/38). Apply the three original migrations (`20260918100000`, `20260918101000`, `20260918110000`) plus `20260918162936_audit_author_edits_replies_and_analytics.sql` before deploying their matching code; `docs/ops/check-remote-migrations.sql` provides signature checks, not proof of complete production migration state. This completion adds no migration. Production launch is not signed off: production-shaped load/accessibility/old-client validation, approved provider/blocklist configuration, moderator staffing/capabilities, retention approval, alerts/AASA/Apple setup, rollback practice, and counsel/business approval remain owner gates. The executable checks and manual evidence matrix are in [`docs/ops/social-release-validation.md`](ops/social-release-validation.md). Community video, algorithmic ranking, additional-language release, and exact crash-free rates are not silently marked complete.

## 40. Implementation references

**Completion verification (September 18, 2026):** 358 unit tests, web/test type checks, lint, production build, web extraction/key/parameter guards, and the unchanged 1,319-key iOS catalog check passed. An extraction review compared all 225 refactored screen/shared-copy modules with the pre-refactor revision and found English/protocol literal parity. All 14 prerendered pages passed a smoke check for leaked message keys, interpolation placeholders, and escaped arrow entities. No database migration or Swift implementation changed in this completion, so the SQL/iOS build results below refer to the preceding audit, not a new production validation. The manual evidence matrix remains unverified.

**Audit verification (September 18, 2026):** the complete migration history, including the audit follow-up, replayed successfully on a clean isolated database. All 68 rollback-only SQL test files and 353 unit tests passed. Web type checks, lint, production build, iOS simulator build, and the 1,319-key string-catalog completeness check passed. Database lint reported no errors (existing unused-variable/implicit-cast warnings remain). Production security/performance advisor checks could not be completed because the connected Supabase account denied access. Real-device camera, VoiceOver/Dynamic Type, production load, and external launch approvals are not validated by these build/test results.
