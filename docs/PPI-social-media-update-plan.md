...
**Implementation status (September 18, 2026):** the web and iOS composers show the anti-scam caution as soon as a draft contains an email or a phone-shaped number (`src/lib/messages/contact-warning.ts`, `ContactDetailsDetector`); it is a caution, never a block, and prices or mileage do not trigger it. Delete-for-self is not implemented: messages are retained for both participants under the existing evidence rules, and a member removes a thread from view by blocking or by account deletion.
...
**Implementation status (September 15, 2026):** web copy that the server produces or that shared controls render — report reason labels, policy and enforcement wording, moderation and reporter notifications, safety notices, username rules, notification categories, and the report / gallery / upload / share controls — resolves through the message catalog in `src/lib/i18n` (`messages/en.ts`, `t()` / `translate()`, `useTranslator()` for client components, `getRequestLocale()` for Accept-Language negotiation); backend codes stay stable and the catalog supplies the label. A unit test fails the build when a catalog-owned module embeds a sentence or references a missing key. iOS: SwiftUI literals are extracted by the compiler (`SWIFT_EMIT_LOC_STRINGS`) into `Localizable.xcstrings` (1,300+ strings), app-produced `String` copy uses `String(localized:)`, shared components take `LocalizedStringKey`, and `scripts/sync-string-catalog.py --check` keeps the catalog complete in CI. Remaining screen-level web copy (page headings, form labels, empty states outside the shared controls) is still literal JSX; it migrates page by page into the same catalog with no infrastructure change, and the first non-English catalog is the trigger to switch the root layout to per-request locale negotiation.
...
**Repository status (September 18, 2026, audited):** the September 18 work adds author editing, one-level replies, soft comment removal, the contact-sharing caution, and engagement observations. The audit adds atomic edit/moderation publication, AI-flag enforcement, author-edit rate limits, group-eligibility rechecks, open-case removal protection, top-level-only Helpful marks, current-access checks for reply notifications, revision-correct audit records, successful-save upload metrics, correlated funnels, bounded client analytics, and account-aware iOS health reporting. Apply the three original migrations (`20260918100000`, `20260918101000`, `20260918110000`) plus `20260918162936_audit_author_edits_replies_and_analytics.sql` before deploying the matching code; `docs/ops/check-remote-migrations.sql` checks their presence. Do not treat this as complete launch sign-off: exact session-attributed crash-free measurement is not implemented, page-level web localization remains partial (32.2), and delete-for-self is not offered (22.3). Production-shaped load/accessibility/old-client validation, approved provider/blocklist configuration, moderator staffing/capabilities, retention approval, alerts/AASA/Apple setup, rollback practice, and counsel/business approval still require their named owners. Community video and algorithmic ranking remain deliberately disabled.
