...
**Implementation status (September 18, 2026):** the five measures that had no source data now have it, on the same closed allowlist with the same opt-out, 90-day expiry, and aggregate-only output (`get_product_engagement_signals`, shown on the admin analytics page). Group discovery-to-join counts non-members who opened a group (`group_detail_viewed`, once per group, web and iOS) and how many joined a group afterwards. Upload completion compares reservations issued by the upload routes (`media_upload_reserved`) with reservations a media row claimed (`media_upload_attached`). Unwanted-contact attribution counts profile and message reports (`unwanted_contact_reported`, per 1,000 messages sent) and contact attempts — a conversation, a message, or a friend request — that a block refused (`blocked_contact_attempt`; the member is never told a block applied). Crash-free sessions come from the iOS app: one `app_session_started` per foreground while signed in and one `app_crash_detected` per MetricKit crash diagnostic, with no stack, device, or timing data collected. Intent segmentation is observed, not declared — technician by role, then owner (Garage action), shopper (Marketplace or search), enthusiast (Community) from the first seven days — and D7/D30 meaningful return is reported per segment, with rates withheld under five accounts. Web sessions and web crashes are not measured.
...
**Repository status (September 18, 2026):** the planned implementation scope is represented in web, iOS, migrations, workers, and automated tests, including the deferred-by-policy switches for Community video and explainable ranking. The September 18 pass closed the last product-flow gaps this document still described: author editing of posts and comments with the "Edited" label (14.6, 15.1), one-level comment replies, soft comment removal with the placeholder, the contact-sharing caution in Messages (22.3), and source data for the five section-34 measures. Three migrations must be applied before the matching code deploys (`20260918100000` enum value, `20260918101000` edits/replies, `20260918110000` analytics; `docs/ops/check-remote-migrations.sql` reports which are present), because the feed query now selects the new comment columns. Remaining work is release validation or external ownership rather than an unimplemented product flow: complete the production-shaped load/accessibility/old-client matrix, configure approved provider and blocklist values, provision moderator capabilities and staffing, set the approved retention period, verify production alerts/AASA/Apple credentials, practice rollback, obtain the listed policy/counsel approvals, and — when a second language is scheduled — migrate the remaining page-level web copy (32.2) into the message catalog. Community video and algorithmic ranking remain deliberately disabled until their separate re-approval criteria are met.
