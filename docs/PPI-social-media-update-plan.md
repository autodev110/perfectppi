...
**Implementation status (September 13, 2026):** the main Community feed, every unified-search result type, the dedicated People and Marketplace directories, saved Community/Marketplace content, and group posts/search/members/FAQ now use bounded, stable cursor pagination on web and iOS while retaining legacy offset responses for installed app compatibility. Feed pages preserve visibility, mutes, filters, and repost collapsing; search cursors preserve relevance ordering and are bound to the normalized query and result type. Saved-content keysets preserve private save order, group cursors are bound to the group and normalized query, and Marketplace cursors preserve every browse filter and all six sort modes. iOS supports incremental loading/retry and both clients show immediate loading states. Production-shaped load testing, query/storage telemetry, image variant tuning, and operational job alerts remain open.
