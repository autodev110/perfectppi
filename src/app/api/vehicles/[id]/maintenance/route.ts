...
import { getOwnedVehicleTimelines, maintenanceEventSchema, ownerCanManageVehicle } from "@/features/vehicles/timelines";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ data: timelines.maintenance });
  } catch {
    return NextResponse.json({ error: "The maintenance timeline is temporarily unavailable" }, { status: 500 });
  }
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if ("response" in auth) return auth.response;
  const { id } = await params;
  const parsed = maintenanceEventSchema.safeParse(await request.json().catch(() => null));
  ...
  const { data, error } = await admin
    ...
    .select()
    .single();
  if (error) return NextResponse.json({ error: "The maintenance event could not be saved" }, { status: 500 });
