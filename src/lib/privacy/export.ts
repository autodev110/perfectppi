...
export async function buildAccountDataExport(profileId: string, user: User) {
  ...
  const [
    ...
    technicianProfile,
    writtenReviews,
    submittedServiceDisputes,
    serviceDisputeActions,
    notifications,
    ...
    rows("submitted inspection service disputes", admin.from("ppi_service_disputes").select("*").eq("requester_id", profileId)),
    rows("inspection service dispute actions", admin.from("ppi_service_dispute_events").select("*").eq("actor_id", profileId)),
  ...
  return {
    ...
    technician: {
      ...
      serviceDisputesSubmitted: submittedServiceDisputes,
      serviceDisputeActions,
    },
