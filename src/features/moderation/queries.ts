...
export async function getModerationQueue(status: "pending_review" | "rejected" | "legal_hold" | "all" = "pending_review") {
  ...
  const vehicleMediaIds = items
    .filter((item) => item.entity_type === "vehicle_media")
    .map((item) => item.entity_id);
  const [{ data: reports }, { data: communityMedia }, { data: vehicleMedia }] = await Promise.all([
    entityIds.length
    ...
    communityMediaIds.length
      ? admin.from("community_post_media")
        .select("id, media_type, content_type")
        .in("id", communityMediaIds)
      : Promise.resolve({ data: [] }),
    vehicleMediaIds.length
      ? admin.from("vehicle_media")
        .select("id, media_type, content_type")
        .in("id", vehicleMediaIds)
      : Promise.resolve({ data: [] }),
  ]);
  ...
  return items.map((item) => ({
    ...
    media: [...(communityMedia ?? []), ...(vehicleMedia ?? [])]
      .find((entry) => entry.id === item.entity_id) ?? null,
  }));
}
